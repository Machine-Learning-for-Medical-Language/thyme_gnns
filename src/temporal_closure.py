...
dtr_rel_extender = {"BEFORE":  # a before dt
...
dtr_dtr_extender = {"BEFORE": {"AFTER": "BEFORE",  # a before dt, b after dt ==> a before b
		     },
		     "BEFORE/OVERLAP": {"BEFORE/OVERLAP": "OVERLAP",
		     			 "OVERLAP": "OVERLAP",
		     },
		     "OVERLAP": {"BEFORE/OVERLAP": "OVERLAP",
		     		  "OVERLAP": "OVERLAP",
		     },
		     "AFTER": {"BEFORE": "AFTER",
		     },
...
def extrapolate(data):
    ...
    # for i, timex in enumerate(data["timex"]):  # TODO only include timexes that are actually used
    #     e_to_i["timex"][timex] = i
    #     i_to_e["timex"][i] = timex

    def which_is_gold(pair1, pair2, et1, et2):
        ...
        rel1_data = [rel for rel in data["relations"] if rel["arg1"]==rel1_arg1 and rel["arg2"]==rel1_arg2 and rel["category"]==rel1_rel]
        rel1_data.extend([rel for rel in data["relations"] if rel["arg1"]==rel1_arg2 and rel["arg2"]==rel1_arg1 and rel["category"]==reverse_rel1_rel])
        ...
        rel2_data = [rel for rel in data["relations"] if rel["arg1"]==rel2_arg1 and rel["arg2"]==rel2_arg2 and rel["category"]==rel2_rel]
        rel2_data.extend([rel for rel in data["relations"] if rel["arg1"]==rel2_arg2 and rel["arg2"]==rel2_arg1 and rel["category"]==reverse_rel2_rel])
    ...
    def check_non_contradictory(edges):
        for et in edges:
            if et[1] != "OVERLAP":
                ...
                for pair in pairs:
                    ...
                    if pair[0] != pair[1] and torch.any(torch.all(reverse_pairs==pair, dim=1)):
                        gold = which_is_gold(pair1=pair, pair2=pair.flip(dims=[0]), et1=et, et2=et)
                        if gold == 1:
                            # first version is present in original data; remove second
                            edges[et] = edges[et][(edges[et]!=pair.flip(dims=[0]).unsqueeze(1)).any(dim=1)]
                        elif gold == 2:
                            # other way around
                            edges[et] = edges[et][(edges[et]!=pair.unsqueeze(1)).any(dim=1)]
                        else:
                            ...
                            if not (overlap_edges==pair).all(dim=1).any():
                                edges = add_new_edges(edges=edges,
                ...
                if et[0] == et1[0] and et[1] != et1[1] and et[2] == et1[2]:
                    ...
                    for pair in pairs:
                        if torch.any(torch.all(pairs1==pair, dim=1)):
                            ...
                            if et[1] == "OVERLAP" and et1[1].replace("-1", "") in ["BEFORE/OVERLAP", "CONTAINS", "CONTAINS-SUBEVENT", "NOTED-ON", "BEGINS-ON", "ENDS-ON"]:
                                edges[et] = pairs[(pairs!=pair).any(dim=1)].t()
                            elif et1[1] == "OVERLAP" and et[1].replace("-1", "") in ["BEFORE/OVERLAP", "CONTAINS", "CONTAINS-SUBEVENT", "NOTED-ON", "BEGINS-ON", "ENDS-ON"]:
                                edges[et1] = pairs1[(pairs1!=pair).any(dim=1)].t()
                            elif (et[0] == "timex" and et[2] == "DocTime") or (et[0] == "DocTime" and et[2] == "timex"):
                                ...
                                gold = which_is_gold(pair1=pair, pair2=pair, et1=et, et2=et1)
                                if gold == 1:
    ...
    for rel in data["relations"]:
        ...
        edges[(arg1_etype, et, arg2_etype)] = torch.cat((edges[(arg1_etype, et, arg2_etype)], torch.tensor([[arg1_i], [arg2_i]])), dim=1)

        # make bidirectional
        reverse_et = reverse_relation(et)
        edges[(arg2_etype, reverse_et, arg1_etype)] = torch.cat((edges[(arg2_etype, reverse_et, arg1_etype)], torch.tensor([[arg2_i], [arg1_i]])), dim=1)
    for et in edges:
    ...
    for et_ab in edges:
        if len(edges[et_ab][0]) == 0:
            continue
        rel_ab = et_ab[1]
        rel_ab = "CONTAINS" if rel_ab in ["CONTAINS-SUBEVENT", "NOTED-ON"] else rel_ab
        if rel_ab in rel_rel_extender["ab-bc"]:
            for et_bc in edges:
                ...
                if et_ab[2] != et_bc[0]:
                    continue
                rel_bc = et_bc[1]
                rel_bc = "CONTAINS" if rel_bc in ["CONTAINS-SUBEVENT", "NOTED-ON"] else rel_bc  # these are effectively the same
                    ...
                    for _b in edges[et_ab][1]:
                        ...
                        if len(_c) > 0:  # we know len(_a) > 0
                            ...
                            if len(new_srcs_dsts[0]) == 0:
                                ...
                                new_srcs_dsts = torch.cat((new_srcs_dsts, torch.cartesian_prod(_a, _c).t()), dim=1)
                    if len(new_srcs_dsts[0]) > 0:
                        new_srcs_dsts = new_srcs_dsts[(new_srcs_dsts[0] != new_srcs_dsts[1]).repeat(2, 1)].view(2, -1)  # remove self-loops
                        edges = add_new_edges(edges=edges,
                                                et=et_ac,
                                                new_srcs_dsts=new_srcs_dsts)
        if rel_ab in rel_rel_extender["ab-ac"]:
            for et_ac in edges:  # this is functionally the same as above; changing var name for clarity
                ...
                if et_ab[0] != et_ac[0]:
                    continue
                rel_ac = et_ac[1]
                rel_ac = "CONTAINS" if rel_ac in ["CONTAINS-SUBEVENT", "NOTED-ON"] else rel_ac  # effectively the same
                    ...
                    for _a in edges[et_ab][0]:
                        ...
                        if len(_b) > 0 and len(_c) > 0:
                            ...
                            if len(new_srcs_dsts[0]) == 0:
                                ...
                                new_srcs_dsts = torch.cat((new_srcs_dsts, torch.cartesian_prod(_b, _c).t()), dim=1)
                    if len(new_srcs_dsts[0]) > 0:
                        new_srcs_dsts = new_srcs_dsts[(new_srcs_dsts[0] != new_srcs_dsts[1]).repeat(2, 1)].view(2, -1)  # remove self-loops
                        edges = add_new_edges(edges=edges,
                                                et=et_bc,
    ...
    for dtr0 in DTR_TYPES:
        _a = set(edges[("event", dtr0, "DocTime")][0].tolist())
        if len(_a) == 0:
            continue
        # dtr + edge to timex dtr
        for et in dtr_rel_extender[dtr0]:
            _b = set(edges[("event", et, "timex")][1].tolist())
            if len(_b) == 0:
                continue
            new_srcs_dsts = torch.cartesian_prod(torch.tensor(list(_b)), torch.tensor([0])).t()
        ...
        for dtr1 in DTR_TYPES:
            if dtr1 in dtr_dtr_extender[dtr0]:
                ...
                new_srcs_dsts = torch.cartesian_prod(torch.tensor(list(_a)).to(int), torch.tensor([0])).t()
                if len(new_srcs_dsts) > 0:
                    new_srcs_dsts = new_srcs_dsts[(new_srcs_dsts[0] != new_srcs_dsts[1]).repeat(2, 1)].view(2, -1)  # remove self-loops
                    edges = add_new_edges(edges=edges, et=("event", rel_ab, "DocTime"), new_srcs_dsts=new_srcs_dsts)
    for et in edges:
        if edges[et].shape[-1] > 0:
            edges[et] = edges[et].unique(dim=1)  # remove duplicate pairs
            if et[0] == "DocTime":  # make sure doctime only has idx 0; otherwise, have mixed it up with some other node
    ...
    return {et: edges[et] for et in edges if edges[et].shape[-1] > 0}, e_to_i  # remove edge types with no edges


def add_new_edges(edges: Dict[Tuple[str], torch.Tensor], 
                  et: str, 
                  new_srcs_dsts = torch.Tensor
                  ) -> Dict[Tuple[str], torch.Tensor]:
    reverse_et = (et[-1], reverse_relation(et[1]), et[0])
    edges[et] = torch.cat((edges[et], new_srcs_dsts), axis=1)
    edges[reverse_et] = torch.cat((edges[reverse_et], new_srcs_dsts.flip(dims=[0])), axis=1)
    return edges
